...
const ShopSection = ({ cartItems, setCartItems }) => {
  ...
  const products = [
    { id: 1, name: 'AI Writing Pro', price: 29, period: '/Mo', description: 'Generate high-quality content, blogs, and marketing copy in seconds with advanced AI.', features: ['Unlimited AI generations', '50+ writing templates', 'Grammar checker'], badge: 'Best Seller', badgeColor: 'bg-purple-100 text-purple-700', image: '/src/assets/products/resume-writing.png' },
    { id: 2, name: 'Design Templates Pack', price: 49, period: '/One-Time', description: '2000+ premium templates for social media, presentations, and marketing materials.', features: ['2000+ templates', 'Monthly updates', 'Commercial license'], badge: 'Popular', badgeColor: 'bg-blue-100 text-blue-700', image: '/src/assets/products/design-tool.png' },
    ...
    { id: 4, name: 'Automation Toolkit', price: 79, period: '/Mo', description: 'Automate repetitive tasks and streamline your workflow with powerful tools.', features: ['50+ automations', 'API access', 'Custom workflows'], badge: 'New', badgeColor: 'bg-green-100 text-green-700', image: '/src/assets/products/operation.png' },
    { id: 5, name: 'Resume Builder Pro', price: 15, period: '/One-Time', description: 'Create professional resumes and cover letters that land interviews.', features: ['100+ templates', 'ATS optimization', 'Export to PDF'], badge: 'Best Seller', badgeColor: 'bg-purple-100 text-purple-700', image: '/src/assets/products/portfolio.png' },
    { id: 6, name: 'Social Media Content Kit', price: 39, period: '/Mo', description: 'Complete toolkit for creating engaging social media content across all platforms.', features: ['5000+ assets', 'Scheduler included', 'Analytics dashboard'], badge: 'Popular', badgeColor: 'bg-blue-100 text-blue-700', image: '/src/assets/products/social-media.png' }
  ];
  ...
  const handleAddToCart = (product) => {
    ...
    if (isExist) {
      ...
      toast.success(`${product.name} added to cart! 🎉`); // সাকসেস টোস্ট
    }
  };

  // Cart থেকে ডিলিট করার ফাংশন (Toast সহ)
  const handleRemove = (id) => {
    const remainingCart = cartItems.filter((item) => item.id !== id);
    setCartItems(remainingCart);
    toast.error("Item removed from cart! 🗑️"); // রিমুভ টোস্ট
  };
        ...
        {activeTab === 'products' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product) => {
              // চেক করা হচ্ছে প্রোডাক্টটি কার্টে আছে কিনা
